...
void compareMatrices(float* cpuMatrix, float* gpuMatrix, int rows, int cols, char* name) {
    ...
    if (match) printf("Success: %s matrices match within tolerance %.6f\n", name, epsilon);
}

void transposeMatrix(float* inputMatrix, float* transposedMatrix, int rows, int cols) {
...
void computeAttentionCPU(float* query, float* key, float* value, float* attentionScores, float* output) {
    ...
    for (int row = 0; row < NUM_SAMPLES; row++) {
        ...
        for (int col = 0; col < NUM_SAMPLES; col++) attentionScores[row * NUM_SAMPLES + col] /= sumExp;
    }
...
__global__ void shared_compute_scores(float* queryMatrix, float* keyTransposeMatrix, float* attentionScores) {
    ...
    for (int phase = 0; phase < numPhases; phase++) {
        ...
        else sharedQuery[threadY][threadX] = 0.0f;
        ...
        else sharedKeyTranspose[threadY][threadX] = 0.0f;
        ...
        if (scoreColumnIndex < NUM_SAMPLES && scoreRowIndex < NUM_SAMPLES) {
            ...
            for (int i = 0; i < TILE_WIDTH; i++) scoreValue += sharedQuery[threadY][i] * sharedKeyTranspose[i][threadX];
        }
...
__global__ void shared_compute_output(float* softmaxScores, float* valueMatrix, float* outputMatrix) {
    ...
    for (int phase = 0; phase < numPhases; phase++) {
        ...
        else sharedSoftmaxScores[threadY][threadX] = 0.0f;
        ...
        else sharedValueMatrix[threadY][threadX] = 0.0f;
...
void computeAttentionGPUShared(float* queryMatrix, float* keyTransposeMatrix, float* valueMatrix, float* attentionScores, float* outputMatrix) {
    ...
    // Free device memory
    cudaFree(deviceQuery); cudaFree(deviceKeyTranspose); cudaFree(deviceValue);
    cudaFree(deviceAttentionScores); cudaFree(deviceSoftmaxScores); cudaFree(deviceOutput);
}
...
int main() {
    ...
    delete[] queryMatrix; delete[] keyMatrix; delete[] valueMatrix;
    delete[] outputCPU; delete[] outputGPUGlobal; delete[] outputGPUShared;
    delete[] attentionScoresCPU; delete[] attentionScoresGlobal;
    delete[] attentionScoresShared; delete[] transposedKeyMatrix;
